...
{
  return m_invert ^ g_infoManager.GetBoolValue(m_info, item);
}

InfoExpression::InfoAssociativeGroup::InfoAssociativeGroup(
    bool and_not_or,
    InfoSubexpressionPtr &left,
    InfoSubexpressionPtr &right)
...
void InfoExpression::InfoAssociativeGroup::AddChild(InfoSubexpressionPtr &child)
{
  ...
  else
  {
    ...
    else
    {
      ...
      if (right_type == new_type)
      {
        ...
        nodes.push(static_cast<InfoSubexpressionPtr>(new InfoAssociativeGroup(new_type == NODE_AND, left, right)));
...
bool InfoExpression::ProcessOperand(std::string &operand, bool invert, std::stack<node_type_t> &node_types, std::stack<InfoSubexpressionPtr> &nodes)
{
  ...
  unsigned int info = g_infoManager.Register(operand, m_context, &listItemDependent);
  if (info == 0)
    return false;
  m_listItemDependent |= listItemDependent;
  nodes.push(static_cast<InfoSubexpressionPtr>(new InfoLeaf(info, invert)));
